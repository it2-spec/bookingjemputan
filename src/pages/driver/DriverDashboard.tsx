    ...
    <div className="min-h-screen bg-surface-50 dark:bg-surface-950 pb-12">
      ...
      <header className="bg-emerald-700 text-white px-4 py-4 shadow-md sticky top-0 z-30">
        <div className="max-w-4xl mx-auto flex items-center justify-between">
          <div className="flex items-center gap-3">
            ...
            <div>
              <h1 className="text-base font-bold flex items-center gap-2">
                Driver Console <span className="bg-emerald-800 text-[10px] px-2 py-0.5 rounded-full font-mono uppercase">Gojek Style</span>
              </h1>
          ...
          <button
            onClick={logout}
            className="text-xs bg-emerald-800 hover:bg-emerald-900 px-3 py-1.5 rounded-lg border border-emerald-600 font-medium transition-colors"
          >
            Keluar
          </button>
        </div>
      ...
      <main className="max-w-4xl mx-auto px-4 py-6 space-y-6">
        {/* Status Control Card */}
        <div className="bg-white dark:bg-surface-900 rounded-2xl p-5 border border-surface-200 dark:border-surface-800 shadow-sm space-y-4">
          <div className="flex flex-wrap items-center justify-between gap-3 border-b border-surface-100 dark:border-surface-800 pb-3">
            <div>
              ...
              <p className="text-xs text-surface-500">Tentukan lokasi penjemputan real-time untuk penumpang</p>
            </div>
            <button
              onClick={() => {
          ...
          <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
            <div>
              ...
              <select
                ...
                className="w-full px-3 py-2 text-sm bg-surface-50 dark:bg-surface-800 border border-surface-300 dark:border-surface-700 rounded-xl focus:ring-2 focus:ring-emerald-500"
              >
                {routes.map((r) => (
          ...
          <div className="pt-2">
            <span className="text-[11px] text-surface-500 block mb-1 font-medium">Pilih Poin Lokasi Cepat:</span>
            <div className="flex flex-wrap gap-2">
              {Object.keys(STATIONS).map((stName) => (
                <button
                  key={stName}
                  onClick={() => handleSelectPresetStation(stName)}
                  className="px-3 py-1 bg-emerald-50 dark:bg-emerald-950/40 text-emerald-700 dark:text-emerald-300 text-xs font-semibold rounded-lg border border-emerald-200 dark:border-emerald-800 hover:bg-emerald-100 transition-colors flex items-center gap-1 cursor-pointer"
                >
        ...
        {/* Live Map Display */}
        <div className="bg-white dark:bg-surface-900 rounded-2xl p-4 border border-surface-200 dark:border-surface-800 shadow-sm space-y-3">
          ...
          <LiveMap
            ...
            className="h-80 w-full rounded-xl overflow-hidden border border-surface-200 dark:border-surface-800 shadow-inner"
          />
        ...
        <div className="bg-white dark:bg-surface-900 rounded-2xl p-5 border border-surface-200 dark:border-surface-800 shadow-sm space-y-4">
          <div className="flex items-center justify-between border-b border-surface-100 dark:border-surface-800 pb-3">
            <div>
              ...
              <p className="text-xs text-surface-500">Total {filteredBookings.length} penumpang terdaftar hari ini</p>
            </div>
            <button
              onClick={fetchData}
              className="p-2 rounded-xl bg-surface-100 dark:bg-surface-800 text-surface-600 dark:text-surface-300 hover:bg-surface-200 transition-colors"
            >
            ...
            <div className="divide-y divide-surface-100 dark:divide-surface-800">
              {filteredBookings.map((b) => (
                <div key={b.id} className="py-3 flex items-center justify-between">
                  <div className="flex items-center gap-3">
                    <div className="w-8 h-8 rounded-lg bg-emerald-100 dark:bg-emerald-900/50 text-emerald-700 dark:text-emerald-300 font-bold text-xs flex items-center justify-center border border-emerald-200 dark:border-emerald-800">
                      #{b.seat_number}
                    </div>
                    <div>
                      <div className="text-xs font-bold text-surface-900 dark:text-surface-100">
                  ...
                  <div className="flex items-center gap-2">
                    <span className="text-[11px] px-2 py-0.5 rounded-full bg-emerald-100 dark:bg-emerald-950 text-emerald-700 dark:text-emerald-300 font-semibold border border-emerald-200 dark:border-emerald-800 flex items-center gap-1">
